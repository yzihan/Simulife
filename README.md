...
This repo is the raw quantative data and analysis code for project Simulife.

---

### File Structure

- [x] 🍀 `data_scale`: Participant Self-Report Scale Data -- This directory contains the rated scale data collected through self-reports from participants, organized into two settings:
  - [x] `with.xlsx`: Data collected when participants interacted with the sage agent.
  - [x] `without.xlsx`: Data collected when participants interacted with the system without the sage agent.
- [x] 🏁 `data_system_log`: System Interaction Data -- This directory includes logs and data captured from the system backend, detailing user interactions and agent performance:
  - [x] `interaction.xlsx`: This file contains the data of user conversations, and decision-making processes during the interaction and chats with sage agent.
